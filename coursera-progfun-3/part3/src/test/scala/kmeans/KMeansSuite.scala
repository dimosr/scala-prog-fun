...
class KMeansSuite extends FunSuite {
  ...
  def checkKMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double, expectedMeans: GenSeq[Point]): Unit = {
    val actualMeans = kMeans(points, means, eta)
    val check = (actualMeans zip expectedMeans).forall(pair => pair._1.squareDistance(pair._2) < 5)

    assert(
      check,
      s"kMeans($points, $means, $eta) = $actualMeans should be close to $expectedMeans")
  }
  ...
  test("'kMeans should calculate kMeans correctly") {
    val points: GenSeq[Point] = ParSeq(new Point(0, 0, 1), new Point(0,0, -1), new Point(0,1,0), new Point(0,10,0))
    val means: GenSeq[Point] = ParSeq(new Point(0, -1, 0), new Point(0, 2, 0))
    val expectedMeans = ParSeq(new Point(0.0, 0.0, 0.0), new Point(0.0, 5.5, 0.0))
    checkKMeans(points, means, 12.25, expectedMeans)
  }

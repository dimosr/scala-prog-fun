...
package object barneshut {
  ...
  case class Fork(
    ...
    val massX: Float =
      if(mass == 0)
        ...
        quads.map(quad => quad.mass * quad.massX).sum / mass
    val massY: Float =
      if(mass == 0)
        centerY
      else
        quads.map(quad => quad.mass * quad.massY).sum  / mass
    val total: Int = quads.map(_.total).sum

    def insert(b: Body): Fork = {
      if(bodyInQuad(b, nw)) {
        Fork(nw.insert(b), ne, sw, se)
      } else if(bodyInQuad(b, ne)) {
        Fork(nw, ne.insert(b), sw, se)
      } else if(bodyInQuad(b, sw)) {
        Fork(nw, ne, sw.insert(b), se)
      } else if(bodyInQuad(b, se)) {
        Fork(nw, ne, sw, se.insert(b))
      } else {
        //invalid state - provided body outside the Quad so not adding it
        this
      }
    }

    def bodyInQuad(body: Body, quad: Quad): Boolean = {
      body.x >= (quad.centerX - quad.size/2) &&
      body.x <= (quad.centerX + quad.size/2) &&
      body.y >= (quad.centerY - quad.size/2) &&
      body.y <= (quad.centerY + quad.size/2)
    }
  }
  ...
  extends Quad {
    ...
    val massX =
      if(mass == 0)
        bodies.map(b => b.x * b.mass).sum
      else
        bodies.map(b => b.x * b.mass).sum / mass
    val massY =
      if(mass == 0)
        bodies.map(b => b.y * b.mass).sum
      else
    ...
    def insert(b: Body): Quad = {
      if(this.size > minimumSize) {
        ...
        (bodies :+ b).foldLeft(emptyFork)((fork, body) => fork.insert(body))
      } else {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Fork(nw, ne, sw, se) =>
          ...
          if (quad.size / dist < theta) {
            ...
            List(nw, ne, sw, se).foreach ( q => traverse(q))
          }
      }

...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'occurrencesOfLang' should work for (specific) RDD with more than one elements") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val rdd = sc.parallelize(Seq(WikipediaArticle("title1", "Java Jakarta Ruby and more"), WikipediaArticle("title2", "Java Jakarta Python and more")))
    val res = (occurrencesOfLang("Java", rdd) == 2)
    assert(res, "Java should occur twice")
    val res2 = (occurrencesOfLang("Jakarta", rdd) == 2)
    assert(res2, "Jakarta should occur twice")
    val res3 = (occurrencesOfLang("Ruby", rdd) == 1)
    assert(res3, "Ruby should occur once")
    val res4 = (occurrencesOfLang("Python", rdd) == 1)
    assert(res4, "Python should occur once")
  }
